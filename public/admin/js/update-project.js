//let updateProjectBtn = document.querySelector('.articles');
{
  ...
  articlesBlock.addEventListener('click', async function (e) {
    if (e.target.classList.contains('btn-edit')) {
      ...
      let projectInfo = await fetch('http://localhost:3000/admin/' + id)
        .then((res) => res.json())
        .then((data) => data);

      titleInp.value = projectInfo.title;
      textArea.value = projectInfo.text;
